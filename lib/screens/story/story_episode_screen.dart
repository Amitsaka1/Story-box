...
/// Full-screen single-episode reader -- pages (zero gap, exactly like
/// the admin upload preview), pinch-to-zoom/pan via InteractiveViewer,
/// Like/Dislike, and the (story-wide, not per-episode) comment thread,
/// all merged onto one screen, accessible via a bottom sheet so the
/// reading area itself stays fullscreen and uncluttered.
    ...
    return Scaffold(
      ...
      body: sorted.isEmpty
          ...
          : InteractiveViewer(
              ...
              minScale: 1,
              maxScale: 4,
              // Zero gap between pages -- ListView with zero padding and
              // no separators, exactly matching the admin upload preview,
              // so the chapter reads as one continuous strip.
...
/// fullscreen. Same pagination/posting logic as before, just moved out
/// of the main scroll view.
class _CommentsSheet extends StatefulWidget {
  final String storyId;
  final StoryService storyService;
  final ScrollController scrollController;

  const _CommentsSheet({required this.storyId, required this.storyService, required this.scrollController});
...
class _CommentsSheetState extends State<_CommentsSheet> {
  ...
  Future<void> _loadCommentsInitial() async {
    ...
    try {
      final result = await widget.storyService.fetchComments(widget.storyId, page: 1);
      if (!mounted) return;
      setState(() {
        _comments
          ..clear()
  ...
  Future<void> _loadCommentsMore() async {
    ...
    try {
      ...
      final result = await widget.storyService.fetchComments(widget.storyId, page: nextPage);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _submitComment() async {
    ...
    try {
      final comment = await widget.storyService.postComment(widget.storyId, text);
      if (!mounted) return;
      setState(() {
